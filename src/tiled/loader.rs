...
use bevy_playdate::asset::{AssetCache, ResAssetCache};

#[derive(SystemParam)]
pub struct TiledLoader<'w>(ResMut<'w, ResAssetCache>);
...
impl<'w> TiledLoader<'w> {
    ...
    pub fn load_tmx_map(&mut self, path: impl AsRef<ResourcePath>) -> Result<Arc<Map>, Error> {
        if let Some(x) = self.0.0.lock().unwrap().get::<Map>(&path.as_ref()) {
            return Ok(x);
        }
        
        let map = self.loader().load_tmx_map(&path)?;
        let out = Arc::new(map);
        self.0.0.lock().unwrap().insert_arc(path.as_ref().to_string(), out.clone());
...
impl<'a, 'w> ResourceCache for &'a mut TiledLoader<'w> {
    fn get_tileset(&self, path: impl AsRef<ResourcePath>) -> Option<Arc<Tileset>> {
        self.0.0.lock().unwrap().get(path.as_ref())
    }

    fn insert_tileset(&mut self, path: impl AsRef<ResourcePath>, tileset: Arc<Tileset>) {
        self.0.0.lock().unwrap().insert_arc(path.as_ref().to_string(), tileset);
    }
    ...
    fn insert_template(&mut self, path: impl AsRef<ResourcePath>, template: Arc<Template>) {
        self.0.0.lock().unwrap().insert_arc(path.as_ref().to_string(), template);
    }
}